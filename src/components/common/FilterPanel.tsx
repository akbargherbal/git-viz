...
import { X, Filter, User, File, RotateCcw, Search, ChevronDown, ChevronUp } from "lucide-react";
...
const AuthorItem: React.FC<AuthorItemProps> = ({ name, count, isSelected, onToggle }) => (
  <div
    onClick={onToggle}
    className={`
...
const FileTypePill: React.FC<FileTypePillProps> = ({ extension, count, isSelected, onToggle }) => (
  <button
    onClick={onToggle}
    className={`
    ...
    `}
  >
    {extension} <span className="opacity-70 text-xs ml-1">{count.toLocaleString()}</span>
  </button>
);
...
export const FilterPanel: React.FC<FilterPanelProps> = (props) => {
  ...
  const {
    ...
    if (isControlled) {
      ...
      return {
        ...
        hiddenAuthorsCount: (authorSearch.trim() || authorsExpanded)
          ? 0 
  ...
  const hasActiveFilters = selectedAuthors.size > 0 || selectedExtensions.size > 0;
      ...
      <div className="p-4 border-t border-zinc-800 bg-zinc-900/95 backdrop-blur-sm">
        ...
        {hasActiveFilters && (
          <div className="flex justify-between text-xs text-zinc-500 mb-3">
            <div>
              Filters Active: <span className="text-purple-400 font-semibold">{selectedAuthors.size + selectedExtensions.size}</span>
            </div>
          </div>
        )}
