...
export class TreemapPlugin implements VisualizationPlugin<TreemapConfig, any> {
  ...
  processData(dataset: OptimizedDataset): d3.HierarchyNode<OptimizedDirectoryNode> {
    // Assign a default value of 1 to files so the treemap has size
    const root = d3.hierarchy(dataset.tree)
  ...
  render(root: HierarchyRectangularNode<OptimizedDirectoryNode>, config: TreemapConfig): void {
    ...
    const treemap = d3.treemap<OptimizedDirectoryNode>()
      .size([width, height])
      .paddingOuter(3)
      .paddingTop(19)
      .paddingInner(1)
