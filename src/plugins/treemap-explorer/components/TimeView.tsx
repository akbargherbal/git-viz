...
export const TimeView: React.FC<TimeViewProps> = ({ file }) => {
  ...
  const lastModifiedDate = new Date(file.lastModifiedDate).toLocaleDateString(
    "en-US",
    {
      month: "short",
      day: "numeric",
  ...
  const daysSinceModified = Math.floor(
    (new Date().getTime() - new Date(file.lastModifiedDate).getTime()) /
      (1000 * 60 * 60 * 24),
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        className={`flex items-center justify-between p-3 rounded-lg border ${getLifecycleColor(file.isDormant)}`}
      >
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-zinc-900/50 p-3 rounded-lg">
          ...
          <div
            className={`text-sm font-mono ${file.isDormant ? "text-red-400" : "text-green-400"}`}
          >
            ...
            <div className="flex items-end justify-between h-full gap-0.5">
              {file.activityTimeline.map((point, i) => {
                const maxCommits = Math.max(
                  ...file.activityTimeline!.map((p) => p.commits),
                );
      ...
      <div className="space-y-3 pt-4 border-t border-zinc-800">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-mono text-zinc-200">
            {file.ageDays > 0
              ? (file.totalCommits / file.ageDays).toFixed(2)
              : "0.00"}
          </span>
        </div>
      </div>
