...
export const DebtView: React.FC<DebtViewProps> = ({ file }) => {
  ...
  return (
    <div className="space-y-4">
      {/* Health Score Badge */}
      <div className={`flex items-center justify-between p-3 rounded-lg border ${getCategoryBg(healthScore.category)}`}>
        <span className="text-xs text-zinc-400">Health Score</span>
        <div className="flex items-center gap-2">
          <span className={`text-2xl font-bold ${getCategoryColor(healthScore.category)}`}>
            {healthScore.score}
          </span>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={`text-sm font-semibold uppercase ${getCategoryColor(healthScore.category)}`}>
      ...
      <div className="space-y-3 pt-4 border-t border-zinc-800">
        <h4 className="text-xs font-semibold text-zinc-400 uppercase">Contributing Factors</h4>
      ...
      <div className="grid grid-cols-2 gap-3 pt-4 border-t border-zinc-800">
        <div>
          ...
          <div className="text-lg font-mono text-zinc-200">{file.total_commits}</div>
        </div>
        <div>
          <div className="text-xs text-zinc-500 mb-1">Contributors</div>
          <div className="text-lg font-mono text-zinc-200">{file.unique_authors}</div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {file.operations.M !== undefined && (
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono text-amber-400">{file.operations.M}</span>
              </div>
            )}
            {file.operations.A !== undefined && (
              <div className="flex items-center justify-between">
                <span className="text-zinc-500">Added</span>
                <span className="font-mono text-green-400">{file.operations.A}</span>
              </div>
            )}
            {file.operations.D !== undefined && (
              <div className="flex items-center justify-between">
                <span className="text-zinc-500">Deleted</span>
                <span className="font-mono text-red-400">{file.operations.D}</span>
              </div>
            )}
            {file.operations.R !== undefined && (
              <div className="flex items-center justify-between">
                <span className="text-zinc-500">Renamed</span>
                <span className="font-mono text-blue-400">{file.operations.R}</span>
              </div>
            )}
          </div>
        </div>
